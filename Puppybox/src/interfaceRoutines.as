...
private function reveal(cm:String):void
{       	    
	revealState=cm;
	if(edgeCaseCounter>0)
	{
		edgeCaseCounter = 0;
	}
	this.currentState = "win";			   	        	        			
	boxSource = "./assets/images/"+ hiddenItem +".jpeg";	        		      			        			        	
	if(audioQueue.length>0)
	{
		audioQueue.removeAll();
	}
...
{       			
	...
	if(revealState.indexOf("reveal2") != -1)
	{
		addAudio("./assets/conclusion/not right.mp3");
	}
	else
	{
...
private function edge(cm:String):void
{
	if(audioQueue.length>0)
	{
		audioQueue.removeAll();
	}
	if(cm.indexOf("edge2") != -1)
	{
		edgeCaseCounter++;		
		if(edgeCaseCounter>4)
		{
			addAudio("./assets/edge cases/lets switch.mp3");	
		}
		else
		{
			addAudio("./assets/edge cases/i dont know.mp3");			
		}
	}
	else
	{
...
private function game():void
{
	...
	addAudio("./assets/trial loops/what items do we have to choose from.mp3");
	playIntroAudio();		
}
...
private function startGame():void
{
	...
	var tempRand:int = rand(1,100);
	if(tempRand<50)
	{
		hiddenItem = sItem.item1.toString();
	}
	else
	{
		hiddenItem = sItem.item2.toString();	
	}
	sock.sendData("Hidden item is:"+hiddenItem+"\n");
